...
export class RouteTransformer {
  ...
  private handleFirstLevelRoute(
    ...
    if (route.children && route.children.length > 0) {
      converted.children = route.children.map((child) => {
        const childPath = child.path || ''
        let relativePath = childPath.startsWith(parentPath)
          ? childPath.slice(parentPath.length)
          : childPath
        if (relativePath.startsWith('/')) {
          relativePath = relativePath.slice(1)
        }
        return {
          ...this.transform({ ...child, path: relativePath }, 1),
          path: relativePath
        } as ConvertedRoute
      })
    } else {
