    ...
    <div class="qc-detail">
      ...
      <div class="detail-section">
        ...
        <ElDescriptions :column="2" border>
          ...
          <ElDescriptionsItem label="细胞数">{{ rowData?.cellCount ? rowData.cellCount + '个' : '-' }}</ElDescriptionsItem>
          <ElDescriptionsItem label="M0对照孔">{{ rowData?.m0ControlWell || '-' }}</ElDescriptionsItem>
          <ElDescriptionsItem label="CV值">{{ rowData?.cvValue ? rowData.cvValue + '%' : '-' }}</ElDescriptionsItem>
...
<script setup lang="ts">
  defineOptions({ name: 'TissueQcViewDrawer' })
