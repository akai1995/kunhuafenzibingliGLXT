    ...
    <div class="qc-detail">
      ...
      <div class="detail-section">
        ...
        <ElDescriptions :column="2" border>
          ...
          <ElDescriptionsItem label="蜡块号">{{ rowData?.paraffinBlockNo || '-' }}</ElDescriptionsItem>
          <ElDescriptionsItem label="肿瘤细胞数量">{{ rowData?.tumorCellCount || '-' }}</ElDescriptionsItem>
          <ElDescriptionsItem label="肿瘤细胞比例">{{ rowData?.tumorCellRatio ? rowData.tumorCellRatio + '%' : '-' }}</ElDescriptionsItem>
          <ElDescriptionsItem label="样本大小">{{ rowData?.sampleSize || '-' }}</ElDescriptionsItem>
          <ElDescriptionsItem label="有无坏死">{{ rowData?.necrosis || '-' }}</ElDescriptionsItem>
          <ElDescriptionsItem label="有无出血">{{ rowData?.hemorrhage || '-' }}</ElDescriptionsItem>
          <ElDescriptionsItem label="样本种类">{{ rowData?.sampleCategory || '-' }}</ElDescriptionsItem>
          ...
          <ElDescriptionsItem label="重借病理号">{{ rowData?.reBorrowPathologyNo || '-' }}</ElDescriptionsItem>
          <ElDescriptionsItem label="质控人">{{ rowData?.qcOperator || '-' }}</ElDescriptionsItem>
...
<script setup lang="ts">
  defineOptions({ name: 'PathologyQcViewDrawer' })
