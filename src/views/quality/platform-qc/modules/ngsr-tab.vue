...
<template>
  <div>
    <NgsrSearchForm @search="handleSearch" @reset="handleResetSearch" />
    ...
    <ElCard class="art-table-card">
      ...
      >
        ...
        <template #preLibQc="{ row }">
          <QcStatusTag :status="row.preLibQc" clickable @click="handleQcClick(row, 'preLib')" />
        </template>
    ...
    <!-- 查看抽屉 -->
    <NgsrViewDrawer v-model:visible="viewDrawerVisible" :row-data="currentRow" />
...
<script setup lang="ts">
  ...
  const selectedRows = ref<NgsrItem[]>([])
  ...
  const ngsrProjects = ['NGSR质控']
  const projects = ngsrProjects
  ...
  // NGSR质控顺序：核酸质控 → 预文库质控 → 终文库质控 → 上机质控 → 生信审核
  const statusGroups: Array<[string, string, string, string, string]> = [
    ['合格', '合格', '合格', '合格', '合格'],
    ['合格', '合格', '合格', '合格', '合格'],
    ['合格', '合格', '合格', '合格', '合格'],
    ['合格', '合格', '合格', '合格', '合格'],
  ...
  const mockData: NgsrItem[] = Array.from({ length: 25 }, (_, i) => {
    const id = i + 1
    const [na, preLib, finalLib, seq, bio] = statusGroups[i]
    return {
      id,
      qcTime: `2026-07-${String(2 + Math.floor(i / 5)).padStart(2, '0')} 0${String((i % 5) + 1).padStart(2, '0')}:00`,
      pathologyNo: `BL${String(20261000 + id)}`,
      sampleNo: `S${String(2026070000 + id)}`,
  ...
  const allData = ref<NgsrItem[]>([...mockData])
  const data = ref<NgsrItem[]>([])
  ...
  const columns = ref([
    ...
    { prop: 'preLibQc', label: '预文库质控', width: 100, useSlot: true },
  ...
  const fetchPageData = () => {
    loading.value = true
    setTimeout(() => {
      // 流程三：从Store获取样本质控完成的流转样本，只合并NGSR项目的样本
      const flowSamples = sampleFlowStore.getSamplesForPlatformQc().filter((fs) =>
        ngsrProjects.includes(fs.projectName)
      )
      ...
      const filtered = allData.value.filter((item) => {
        return (
          ...
          (!searchParams.preLibQc || item.preLibQc === searchParams.preLibQc) &&
  ...
  const handleSelectionChange = (selection: NgsrItem[]) => {
    selectedRows.value = selection
  }

  const handleEdit = (row: NgsrItem) => {
    currentRow.value = { ...row }
    currentQcType.value = ''
    editDrawerVisible.value = true
  }

  const handleQcClick = (row: NgsrItem, qcType: string) => {
    const { passed, message } = checkQcSequence(qcType, row, true)
    if (!passed) {
