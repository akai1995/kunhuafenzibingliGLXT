    ...
    <div class="qc-detail">
      ...
      <div class="detail-section">
        ...
        <ElDescriptions :column="2" border>
          ...
          <ElDescriptionsItem label="是否离心">{{ rowData?.isCentrifuged || '-' }}</ElDescriptionsItem>
          <ElDescriptionsItem label="体积">{{ rowData?.volume || '-' }}</ElDescriptionsItem>
          <ElDescriptionsItem label="全血体积">{{ rowData?.wholeBloodVolume || '-' }}</ElDescriptionsItem>
          <ElDescriptionsItem label="血浆体积">{{ rowData?.plasmaVolume || '-' }}</ElDescriptionsItem>
          <ElDescriptionsItem label="有无溶血">{{ rowData?.hemolysis || '-' }}</ElDescriptionsItem>
          <ElDescriptionsItem label="有无脂血">{{ rowData?.lipidemia || '-' }}</ElDescriptionsItem>
          <ElDescriptionsItem label="有无凝血">{{ rowData?.coagulation || '-' }}</ElDescriptionsItem>
...
<script setup lang="ts">
  defineOptions({ name: 'BloodQcViewDrawer' })
