...
<script setup lang="ts">
  ...
  const selectedRows = ref<DnaItem[]>([])
  ...
  const names = ['张明', '李华', '王芳', '刘强', '陈静', '杨光', '赵丽', '周伟', '吴刚', '郑红']
  const genders = ['男', '女']
  const nations = ['汉族', '彝族', '白族', '回族']
  const projects = ['肺癌10基因检测', '结直肠癌8基因检测', '乳腺癌6基因检测', '甲状腺癌5基因检测']
  const sampleTypes = ['组织样本', '血液样本', '穿刺液']
  const qcResults = ['合格', '合格', '合格', '合格', '合格', '合格', '合格', '不合格', '合格', '待质控']
  const extractStatuses = ['已提取', '已提取', '已提取', '已提取', '已提取', '已提取', '已提取', '已提取', '未提取', '未提取']
  const leaders = ['王芳', '刘强', '陈静', '杨光']

  const mockData: DnaItem[] = Array.from({ length: 10 }, (_, i) => ({
    id: i + 6001,
    patientName: names[i],
    sampleNo: `S${String(2026076000 + i)}`,
    extractStatus: extractStatuses[i],
    extractTime: extractStatuses[i] === '已提取' ? `2026-07-${String(4 + Math.floor(i / 5)).padStart(2, '0')} 0${String((i % 5) + 1).padStart(2, '0')}:00` : '-',
    paraffinBlockNo: `L${String(202600 + i)}`,
    gender: genders[i % 2],
    age: 30 + i * 5,
  ...
  const allData = ref<DnaItem[]>([...mockData])
  const data = ref<DnaItem[]>([])
  ...
  const handleSelectionChange = (selection: DnaItem[]) => { selectedRows.value = selection }

  const handleExportTemplate = () => {
    ElMessage.success(`已导出 ${selectedRows.value.length} 条DNA提取模板数据`)
  }
